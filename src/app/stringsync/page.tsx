...
export default function StringSyncPage() {
    ...
    useEffect(() => {
        ...
        const init = async () => {
            try {
                // Preload audio
                audioEngine.current.preload().catch(console.error);
                // Init HandTracker
    ...
    // Start Camera ONLY when dependencies are loaded AND video ref is available
    useEffect(() => {
        if (isCtxLoaded && videoRef.current && status === 'initializing') {
            const startCamera = async () => {
                try {
                    if (handTracker.current && videoRef.current) {
                        await handTracker.current.start(videoRef.current);
                        setStatus('ready');
                    }
                } catch (err) {
    ...
    }, [isCtxLoaded, status]); // Run when ctx loads and videoRef is available
            ...
            <div className="hidden lg:flex flex-col min-h-screen p-8 max-w-7xl mx-auto">
                ...
                <header className="flex items-center justify-between mb-8">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-2xl font-bold bg-gradient-to-r from-purple-600 via-pink-600 to-red-600 dark:from-purple-400 dark:via-pink-500 dark:to-red-500 bg-clip-text text-transparent">
                            StringSync
                        </h1>
                    </div>
                ...
                <main className="flex-1 flex flex-col items-center justify-center w-full">
                    ...
                    {/* Webcam Container */}
                    <div className={`relative w-full max-w-4xl aspect-[4/3] bg-slate-100 dark:bg-gray-900/50 rounded-3xl overflow-hidden border transition-all duration-100 shadow-2xl backdrop-blur-sm mb-12 group ${Date.now() - lastStrum < 150 ? 'border-purple-500 shadow-purple-500/20 scale-[1.01]' : 'border-slate-200 dark:border-white/10'
                        }`}>
                        ...
                        {/* Loading State */}
                        {!streamReady && (
                            <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-500 bg-slate-50/90 dark:bg-gray-900/80 z-10 transition-colors">
                        ...
                        {/* Strum Flash */}
                        <div className={`absolute inset-0 bg-white/20 dark:bg-white/10 pointer-events-none transition-opacity duration-75 ${Date.now() - lastStrum < 100 ? 'opacity-100' : 'opacity-0'
                            }`} />
